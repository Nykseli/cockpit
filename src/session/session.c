  ...
  for (i = 0; i < num_msg; i++)
    {
      if (msg[i]->msg_style == PAM_PROMPT_ECHO_OFF &&
      ...
      else
        {
          ...
          //
          debug ("prompt for more data because prompt_token is null");
              ...
              const char *authorization)
{
  ...
  char *password = NULL;
  char *user = NULL;
  // char *app_data[2] = {NULL, NULL};
  pam_conv_data app_data;
  int res;
  ...
  debug("passkey authorization: %s", authorization);
      ...
      char **argv)
{
  ...
  debug("authorization raw: %s", authorization);
